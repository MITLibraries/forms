...
?>
<div style="background-color: #4DCC0D; padding: 0.5em;">
<p>This interface is coming from the new production server. Please test by conducting any needed administrative tasks, and provide feedback to Matt Bernhardt at mjbernha@mit.edu.</p>
<p>Thank you!</p>
</div>
<table>
<thead>
...
while ($resultrow = mysql_fetch_assoc($resource)) {
  ...
  echo '</td>';
  echo '<td>';
  echo '<a href="http://libraries.mit.edu/hcl/admin/viewapp.php?app_number=' . $resultrow['app_number'] . '">View/Process</a></td>';
  ...
  if($resultrow['decision'] != '')
  {
      ...
      echo ' (<a href="http://libraries.mit.edu/hcl/admin/processapp.php?app_number=' . $resultrow['app_number'] . '">make PDF</a>)';
  }
  echo '</td>';
  echo '</tr>';
}
...
$sql = "select source, status, department1, department2, count(*) from hcl.applications where decision='approved' group by status, department1, department2";
$resource = mysql_query($sql);
...
$sql = "select source, status, department1, department2, count(*) from hcl.applications where decision='approved' and decisiondate between '2009-07-15' and '2009-07-30' group by status, department1, department2";
$resource = mysql_query($sql);
